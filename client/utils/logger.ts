...
export function logStoreState(state: {
    ...
    logger(
        `Store: phase=${state.phase} player=${state.playerScore}/${state.playerHandCount} dealer=${state.dealerScore}/${state.dealerHandCount} pool=${state.poolCount}`,
        colorStore,
    );
}
